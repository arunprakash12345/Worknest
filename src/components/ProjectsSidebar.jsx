  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        ...
        const formattedProjects = data.map((batch) => ({
          id: batch._id,
          name: batch.title,
        }));
  ...
  const getProjectSubItems = (projectId) => [
    {
      title: "Tasks",
      icon: KanbanIcon,
      url: `/batches/${projectId}?tab=tasks`,
    },
    {
      title: "Analytics",
      icon: ChartColumnIcon,
      url: `/batches/${projectId}?tab=analytics`,
    },
    {
      title: "Calendar",
      icon: CalendarIcon,
      url: `/batches/${projectId}?tab=calendar`,
    },
  ];
            ...
            {expandedProjects.has(project.id) && (
              <div className="ml-5 mt-1 space-y-1">
                {getProjectSubItems(project.id).map((subItem) => {
                  const isActive =
                    location.pathname === `/batches/${project.id}` &&
